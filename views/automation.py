    ...
    with tab_dashboard:
        ...
        with col1:
            if not status.get("running"):
                if st.button("▶️ Start Auto Mode", key="start_auto"):
                ...
                if st.button("⏹️ Stop Automation", key="stop_auto"):
    ...
    with tab_settings:
        ...
        with col2:
            ...
            max_pos = st.slider(
                ...
                float(settings.get("max_position_pct", 5.0)), step=0.5
            )
            max_dd = st.slider(
                "Max Drawdown %", 0.0, 100.0,
                float(settings.get("max_drawdown", 10.0)), step=0.1
            )
